...
def login(username: str, token: str, registry:str="https://index.docker.io/v1",
          engine: str="docker"):
    """
    Log in to a container registry using CLI.

    This function checks if the user is already logged in to the specified container registry by examining the specific engine configuration file.
    If not already logged in, it uses the CLI to log in to the registry using the provided username and password/token
...
def x():
    ...
    # if not cr_token:
    #     raise ValueError("Error: CR_TOKEN environment variable not set - it must be set with a valid token/password for logging in to the registry.")
